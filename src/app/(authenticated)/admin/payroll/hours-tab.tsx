...
export function HoursTab() {
  ...
  // The week we currently hold data for. null until the first load lands.
  const [loadedWeek, setLoadedWeek] = useState<string | null>(null);
  // Bumped to force a reload of the same week after a save or an import.
   ...
   *
   * This used to be a useState that the effect set to true synchronously on every
   * run, which triggers a cascading render — and left two sources of truth that
   * could disagree if a fetch threw. Comparing the week we have data for against
   * the week we are showing cannot get out of sync.
   */
  const loading = loadedWeek !== weekStartStr;

  useEffect(() => {
    // Guards against a slow response for a week you have already navigated away
    // from landing after a fast one and overwriting it.
    let cancelled = false;
    ...
    (async () => {
      ...
      if (expRes.ok) {
        ...
        if (!cancelled) setExpectedMap(map);
      }
      // Every setState above happens after an await, so none of them run
      // synchronously during the effect.
      if (!cancelled) setLoadedWeek(weekStartStr);
    })();
  ...
  /** Re-run the load. Bumping a counter is an event-handler setState, not an effect one. */
  ...
  return (
    <div className="space-y-6">
      ...
      <PayrollUploadCard onImported={refresh} />
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {employees.map((emp) => (
            <EmployeePayrollRow
              /*
               * The week and refresh counter are part of the key on purpose.
               * Changing either remounts the row, which re-runs its useState
               * initializers against the new entry — this is what replaces the
               * nine-setState reset effect that used to live inside it.
               */
              key={`${emp.id}-${weekStartStr}-${refreshKey}`}
              employee={emp}
...
function EmployeePayrollRow({
  ...
  // Read-only here: there is no notes input, but the value must survive a save.
  const [notes] = useState(entry?.notes || '');

  // No effect resets these fields any more.
  //
  // Re-syncing nine useStates from props in an effect meant a second render pass
  // every time the week changed, and it was the second setState-in-effect the
  // linter flagged. The parent now gives each row a key that includes the week
  // and the refresh counter, so React remounts the row and every useState
  // initializer above re-runs with the new entry. Same behaviour, one render,
  // and no way for the reset to fight with what someone is typing.
