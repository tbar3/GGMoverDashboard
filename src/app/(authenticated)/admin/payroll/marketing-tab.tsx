...
import { MarketingForm } from './marketing/marketing-form';
...
export async function MarketingTab({ weekStart: weekParam }: { weekStart: string | null }) {
  const weekStart = weekParam && /^\d{4}-\d{2}-\d{2}$/.test(weekParam)
    ? weekStartOf(weekParam)
    : weekStartOf(format(new Date(), 'yyyy-MM-dd'));
